...
class CloudCenterVirtualMachine {
  ...
  */
  ...
  == <div class="pf-v6-c-description-list__text"><span id="span-cloud-vm-status" class="pf-v6-c-label pf-m-red"><span class="pf-v6-c-label__content" id="span-cloud-vm-status-content"><span class="pf-v6-c-label__icon"><i class="fas fa-fw fa-info-circle" aria-hidden="true"></i></span>인스턴스를 찾을 수 없습니다.</span></span></div>
  ...
  */
  createDescriptionListText(id = 'span-cloud-vm-status', status = 'red', description = 'N/A') {
    let description_list__text = $("<div />").addClass('pf-v6-c-description-list__text')
    let span_cloud_vm_status = $("<span></span>").attr('id', id).addClass('pf-v6-c-label').addClass('pf-m-' + status);
    let span_content = $("<span></span>").addClass("pf-v6-c-label__content").attr('id', "span-cloud-vm-status-content");
    var span_icon = "";
    if(status == 'green'){
      span_icon = $("<span />").addClass("pf-v6-c-label__icon").prepend($('<i class="fas fa-fw fa-check-circle" aria-hidden="true"></i>'));
    }else{
      span_icon = $("<span />").addClass("pf-v6-c-label__icon").prepend($('<i class="fas fa-fw fa-exclamation-triangle" aria-hidden="true"></i>'));
    }
    ...
    return new Promise((resolve) => {
      ...
      vms.forEach(function (vm) {
        if (vm.Name == ccvm_instance.Name) {
          ...
          applyCardStatusTone($('#ccvm-low-info'), "ok");
          // $('#span-cloud-vm-status').attr('class','pf-v6-c-label pf-m-green');
          ...
          applyCardStatusTone($('#ccvm-low-info'), "danger");
    ...
    return new Promise((resolve) => {
      ...
      */
      ...
      if (obj.code == 200) {
        ...
        if (os_type != "ablestack-standalone"){
          var remotePcsStatusFull = ['/usr/bin/ssh', '-o', 'StrictHostKeyChecking=no', ccvm_instance.runningHost, '/usr/bin/python3', pluginpath +'/python/host/virshlist.py'];
          cockpit.spawn(remotePcsStatusFull, { host: pcs_exe_host})
            .then(ccvm_instance.checkVIRSHOK)
            .catch(ccvm_instance.checkVIRSHERR)
        }else{
          var remotePcsStatusFull = ['/usr/bin/python3', pluginpath +'/python/host/virshlist.py'];
          cockpit.spawn(remotePcsStatusFull)
            .then(ccvm_instance.checkVIRSHOK)
            .catch(ccvm_instance.checkVIRSHERR)
        }
        resolve();
  ...
  */
  checkCCVM() {
    ccvm_instance= new CloudCenterVirtualMachine()
    let status_span = $("#description-cloud-vm-status");
    let a = ccvm_instance.createDescriptionListText("span-cloud-vm-status", 'orange', "상태 체크 중 &bull;&bull;&bull;&nbsp;&nbsp;&nbsp;<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
    status_span[0].children[0].replaceWith(a);
    if(os_type != "ablestack-standalone"){
