...
function objectGatewayList(){
  //조회
  $('#button-object-gateway-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
    ...
    if(data.length != 0){
      fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/glue/hosts',{
        ...
        for(var i=0; i < data.length; i++){
          ...
          insert_tr += '  <td class="pf-v6-c-table__icon" role="cell" data-label="편집">';
          insert_tr += '    <div class="ablestack-menu-dropdown">';
          insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-object-gateway'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
          insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
          insert_tr += '      </button>';
          insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-object-gateway'+i+'" id="dropdown-menu-card-action-object-gateway'+i+'">';
          insert_tr += '        <li>';
          insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayEdit("'+data[i].service_name+'")\' >Object Gateway 수정</button>';
          insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayDelete("'+data[i].service_name+'")\' >Object Gateway 삭제</button>';
          insert_tr += '        </li>';
          insert_tr += '      </ul>';
          insert_tr += '    </div>';
          insert_tr += '  </td>';
...
function objectGatewayUserList(){
  //조회
  $('#button-object-gateway-user-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/rgw/user',{
    ...
    if(data != null && data.length != 0){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '  <td class="pf-v6-c-table__icon" role="cell" data-label="편집">';
        insert_tr += '     <div class="ablestack-menu-dropdown">';
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-object-gateway-user'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-object-gateway-user'+i+'" id="dropdown-menu-card-action-object-gateway-user'+i+'">';
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayUserEdit("'+data[i].user_id+'")\' >User 수정</button>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayUserDelete("'+data[i].user_id+'")\' >User 삭제</button>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayUserS3keySearch("'+data[i].user_id+'")\' >User S3 키 조회</button>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayUserQuotaEdit("'+data[i].user_id+'")\' >User Quota 수정</button>';
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
...
function objectGatewayBucketList(){
  //조회
  $('#button-object-gateway-bucket-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/rgw/bucket?detail=true',{
    ...
    if(data != null && data.length != 0){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '  <td class="pf-v6-c-table__icon" role="cell" data-label="편집">';
        insert_tr += '     <div class="ablestack-menu-dropdown pf-m-top">';
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-object-gateway-bucket'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-object-gateway-bucket'+i+'" id="dropdown-menu-card-action-object-gateway-bucket'+i+'">';
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayBucketEdit("'+data[i].bucket+'","'+data[i].id+'")\' >Bucket 수정</button>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'objectGatewayBucketDelete("'+data[i].bucket+'")\' >Bucket 삭제</button>';
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
