...
function iscsiServiceList(){
  //조회
  $('#button-iscsi-service-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/service?service_type=iscsi',{
    ...
    if(data.length != 0 && data.code!=500){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-iscsi-service-status'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-iscsi-service-status'+i+'" id="dropdown-menu-card-action-iscsi-service-status'+i+'">';
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-set-iscsi-service-remove" onclick=\'iscsiServiceEdit("'+data[i].service_name+'")\' >iSCSI 서비스 수정</button>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-set-iscsi-service-remove" onclick=\'iscsiServiceDelete("'+data[i].service_name+'")\' >iSCSI 서비스 삭제</button>';
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
...
$('#button-execution-modal-create-iscsi-service').on('click', function(){
  if(iscsiCreateValidateCheck()){
    ...
    fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/iscsi',{
      method: 'POST',
      headers: {
        'accept': 'application/json',
        'Content-Type': 'application/x-www-form-urlencoded'
      ...
      if(data == "Success"){
        ...
        $("#modal-status-alert-body").html("iSCSI Service 생성을 완료하였습니다.<br/>새로고침 버튼을 클릭하여 서비스 상태를 확인할 수 있습니다.");
...
function iscsiServiceEdit(iscsi_id){
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/service?service_type=iscsi&service_name='+iscsi_id,{
    method: 'GET',
    headers: {
      'accept': 'application/json',
      'Content-Type': 'application/x-www-form-urlencoded'
...
$('#button-execution-modal-update-iscsi-service').on('click', function(){
  if(iscsiUpdateValidateCheck()){
    ...
    $('#div-modal-update-iscsi-service').hide();
    $('#div-modal-spinner-header-txt').text('iSCSI Service를 수정하고 있습니다.');
    $('#div-modal-spinner').show();
  
    $("#modal-status-alert-title").html("iSCSI Service 수정 실패");
    $("#modal-status-alert-body").html("iSCSI Service 수정을 실패하였습니다.");
    ...
    fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/iscsi',{
      ...
      $('#div-modal-spinner').hide();
      if(data == "Success"){
        $("#modal-status-alert-title").html("iSCSI Service 수정 완료");
        $("#modal-status-alert-body").html("iSCSI Service 수정을 완료하였습니다.");
        $('#div-modal-status-alert').show();
        iscsiServiceList();
...
$('#button-execution-modal-remove-iscsi-service').on('click', function(){
  if($('#input-checkbox-iscsi-service-remove').is(":checked")){
    ...
    fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/service/'+iscsi_service_id,{
      method: 'DELETE',
      headers: {
        'accept': 'application/json',
        'Content-Type': 'application/x-www-form-urlencoded'
...
function iscsiTargetList(){
  //조회
  $('#button-iscsi-target-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/iscsi/target',{
    method: 'GET',
    headers: {
      'accept': 'application/json',
      'Content-Type': 'application/x-www-form-urlencoded'
    ...
    if(data.length > 0 || (data.code != undefined && data.code!="no_gateways_defined")){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '  <td class="pf-v6-c-table__icon" role="cell" data-label="편집">';
        insert_tr += '    <div class="ablestack-menu-dropdown">';
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-iscsi-target-status'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-iscsi-target-status'+i+'" id="dropdown-menu-card-action-iscsi-target-status'+i+'">'
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-set-iscsi-target-remove" onclick=\'iscsiTargeEdit("'+data[i].target_iqn+'")\' >iSCSI 타겟 수정</button>'
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-set-iscsi-target-remove" onclick=\'iscsiTargetDelete("'+data[i].target_iqn+'")\' >iSCSI 타겟 삭제</button>'
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
...
function iscsiTargeEdit(iqn_id){
  ...
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/iscsi/target?iqn_id='+iqn_id,{
    method: 'GET',
    headers: {
      'accept': 'application/json',
      'Content-Type': 'application/x-www-form-urlencoded'
...
function iscsiCreateValidateCheck(){
  ...
  } else if (checkForNameDuplicates("iscsi-service-list", 0, 'iscsi.'+service_id)) {
...
async function iscsiTargetCreateValidateCheck(){
  ...
  if (iqn_id == "") {
    alert("IQN을 입력해주세요.");
    validate_check = false;
  } else if (!checkIqn(iqn_id)) {
    alert("IQN 생성 규칙을 확인해주세요.");
    validate_check = false;
