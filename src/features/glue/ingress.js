...
function ingressList(){
  //조회
  $('#button-ingress-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/service?service_type=ingress',{
    ...
    if(data.length != 0){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-ingress'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-ingress'+i+'" id="dropdown-menu-card-action-ingress'+i+'">';
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-ingress-remove" onclick=\'ingressEdit("'+data[i].service_name+'")\' >INGRESS 서비스 수정</button>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-ingress-remove" onclick=\'ingressDelete("'+data[i].service_name+'")\' >INGRESS 서비스 삭제</button>';
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
  ...
  if(ingressCreateValidateCheck()){
    ...
    fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/ingress',{
      ...
      if(data == "Success"){
        ...
        $("#modal-status-alert-body").html("INGRESS Service 생성을 완료하였습니다.<br/>새로고침 버튼을 클릭하여 서비스 상태를 확인할 수 있습니다.");
