...
function nvmeofServiceList(){
  //조회
  $('#button-nvmeof-service-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/service?service_type=nvmeof',{
    ...
    if(data.length != 0){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-nvmeof-service-status'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-nvmeof-service-status'+i+'" id="dropdown-menu-card-action-nvmeof-service-status'+i+'">';
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-set-nvmeof-service-remove" onclick=\'nvmeofServiceDelete("'+data[i].service_name+'")\' >NVMe-oF 서비스 삭제</button>';
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
...
function nvmeofTargetList(){
  //조회
  $('#button-nvmeof-target-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/nvmeof/target',{
    ...
    if(data.length > 0 || (data.code != undefined && data.code!="no_gateways_defined")){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '  <td class="pf-v6-c-table__icon" role="cell" data-label="편집">';
        insert_tr += '    <div class="ablestack-menu-dropdown">';
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-nvmeof-target-status'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-nvmeof-target-status'+i+'" id="dropdown-menu-card-action-nvmeof-target-status'+i+'">'
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-set-nvmeof-target-remove" onclick=\'nvmeofTargetDelete("'+data[i].nqn+'")\' >NVMe-of 타겟 삭제</button>'
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
