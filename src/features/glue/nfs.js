...
function nfsClusterList(){
    //조회
    fetch('https://10.10.2.12:8080/api/v1/nfs',{
        method: 'GET',
        headers: {
        ...
        for(var i=0; i < nfs_cluster_names.length; i++){
            ...
            if(data[nfs_cluster_names[i]].backend.length > 0){
                insert_tr += '    <td role="cell" data-label="호스트 명" id="nfs-cluster-host-name">'+data[nfs_cluster_names[i]].backend[0].hostname+'</td>';
                insert_tr += '    <td role="cell" data-label="IP" id="nfs-cluster-ip">'+data[nfs_cluster_names[i]].backend[0].ip+'</td>';
...
$('#button-execution-modal-create-nfs-cluster').on('click', function(){
    ...
    fetch('https://10.10.2.12:8080/api/v1/nfs/'+nfs_cluster_id+'/'+nfs_cluster_port,{
        method: 'POST',
        headers: {
            'accept': 'application/json',
            'Content-Type': 'application/x-www-form-urlencoded'
...
$('#button-execution-modal-remove-nfs-cluster').on('click', function(){
    ...
    fetch('https://10.10.2.12:8080/api/v1/nfs/'+nfs_cluster_id,{
        method: 'DELETE',
        headers: {
            'accept': 'application/json',
            'Content-Type': 'application/x-www-form-urlencoded'
