...
function nfsClusterList(){
  //조회
  $('#button-nfs-cluster-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
      ...
      fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/service?service_type=nfs',{
        ...
        if(data.length != 0){
          ...
          for(var i=0; i < nfs_cluster_names.length; i++){
            ...
            insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-nfs-cluster'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
            insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
            insert_tr += '      </button>';
            insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-nfs-cluster'+i+'" id="dropdown-menu-card-action-nfs-cluster'+i+'">';
            insert_tr += '        <li>';
            insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'nfsClusterEdit("'+nfs_cluster_names[i]+'","'+nfs_port+'")\' >NFS Cluster 수정</button>';
            insert_tr += '          <button class="pf-v6-c-menu__item" type="button" onclick=\'nfsClusterDelete("'+nfs_cluster_names[i]+'")\' >NFS Cluster 삭제</button>';
            insert_tr += '        </li>';
            insert_tr += '      </ul>';
            insert_tr += '    </div>';
            insert_tr += '  </td>';
  ...
  if(nfsServiceCreateValidateCheck()){
    ...
    fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/nfs/'+nfs_cluster_id+'/'+nfs_cluster_port,{
      ...
      if(data == "Success"){
        ...
        $("#modal-status-alert-body").html("NFS Cluster 생성을 완료하였습니다.<br/>새로고침 버튼을 클릭하여 서비스 상태를 확인할 수 있습니다.");
...
function nfsExportList(){
  //조회
  $('#button-nfs-export-search').html("<svg class='pf-v6-c-spinner pf-m-md' role='progressbar' aria-valuetext='Loading...' viewBox='0 0 100 100' ><circle class='pf-v6-c-spinner__path' cx='50' cy='50' r='45' fill='none'></circle></svg>");
  fetch('https://'+glue_api_ip+':'+glue_api_port+'/api/v1/nfs/export',{
    ...
    if(data != null && data.length != 0){
      ...
      for(var i=0; i < data.length; i++){
        ...
        insert_tr += '  <td class="pf-v6-c-table__icon" role="cell" data-label="편집">';
        insert_tr += '     <div class="ablestack-menu-dropdown">';
        insert_tr += '      <button class="pf-v6-c-menu-toggle pf-m-plain" id="card-action-nfs-export'+i+'" aria-expanded="false" type="button" aria-label="Actions">';
        insert_tr += '        <i class="fas fa-ellipsis-v" aria-hidden="true"></i>';
        insert_tr += '      </button>';
        insert_tr += '      <ul class="pf-v6-c-menu ablestack-menu pf-m-align-right" aria-labelledby="card-action-nfs-export'+i+'" id="dropdown-menu-card-action-nfs-export'+i+'">';
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-nfs-export-edit" onclick=\'nfsExportEdit("'+data[i].cluster_id+'","'+data[i].export_id+'")\' >NFS Export 수정</button>';
        insert_tr += '        </li>';
        insert_tr += '        <li>';
        insert_tr += '          <button class="pf-v6-c-menu__item" type="button" id="menu-item-nfs-export-remove" onclick=\'nfsExportDelete("'+data[i].export_id+'","'+data[i].pseudo+'","'+data[i].cluster_id+'")\' >NFS Export 삭제</button>';
        insert_tr += '        </li>';
        insert_tr += '      </ul>';
        insert_tr += '    </div>';
        insert_tr += '  </td>';
