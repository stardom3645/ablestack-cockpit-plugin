...
$(document).ready(function(){
  ...
  $('#div-accordion-storage-vm-device-conifg').attr('hidden', true).hide();
  $('#div-accordion-storage-vm-additional').attr('hidden', true).hide();
  $('#div-accordion-storage-vm-ssh-key').attr('hidden', true).hide();
  ...
  //os type에 대한 마법사 변경 처리
  setScvmOSTypeProcess();
...
$('#button-accordion-storage-vm-device-conifg').on('click', function(){
  if ($('#button-accordion-storage-vm-device-conifg').attr("aria-expanded") == "false") {
    ...
    $('#div-accordion-storage-vm-device-conifg').removeAttr('hidden').fadeIn();
    $('#div-accordion-storage-vm-device-conifg').addClass("pf-m-expanded");
    $('#div-accordion-storage-vm-device-conifg').closest('.pf-v6-c-accordion__item').addClass("pf-m-expanded");
  }
  else {
    ...
    $('#div-accordion-storage-vm-device-conifg').attr('hidden', true).fadeOut();
    $('#div-accordion-storage-vm-device-conifg').removeClass("pf-m-expanded");
    $('#div-accordion-storage-vm-device-conifg').closest('.pf-v6-c-accordion__item').removeClass("pf-m-expanded");
  }
});
...
$('#button-accordion-storage-vm-additional').on('click', function(){
  if ($('#button-accordion-storage-vm-additional').attr("aria-expanded") == "false") {
    ...
    $('#div-accordion-storage-vm-additional').removeAttr('hidden').fadeIn();
    $('#div-accordion-storage-vm-additional').addClass("pf-m-expanded");
    $('#div-accordion-storage-vm-additional').closest('.pf-v6-c-accordion__item').addClass("pf-m-expanded");
  }
  else {
    ...
    $('#div-accordion-storage-vm-additional').attr('hidden', true).fadeOut();
    $('#div-accordion-storage-vm-additional').removeClass("pf-m-expanded");
    $('#div-accordion-storage-vm-additional').closest('.pf-v6-c-accordion__item').removeClass("pf-m-expanded");
  }
});
...
$('#button-accordion-storage-vm-ssh-key').on('click', function(){
  if ($('#button-accordion-storage-vm-ssh-key').attr("aria-expanded") == "false") {
    ...
    $('#div-accordion-storage-vm-ssh-key').removeAttr('hidden').fadeIn();
    $('#div-accordion-storage-vm-ssh-key').addClass("pf-m-expanded");
    $('#div-accordion-storage-vm-ssh-key').closest('.pf-v6-c-accordion__item').addClass("pf-m-expanded");
  }
  else {
    ...
    $('#div-accordion-storage-vm-ssh-key').attr('hidden', true).fadeOut();
    $('#div-accordion-storage-vm-ssh-key').removeClass("pf-m-expanded");
    $('#div-accordion-storage-vm-ssh-key').closest('.pf-v6-c-accordion__item').removeClass("pf-m-expanded");
  }
});
    ...
    .then(function(data){
      ...
      if(reset_storage_center_result.code=="200") { //정상
        ...
        var create_scvm_cloudinit_cmd = ['python3', pluginpath + '/python/vm/create_scvm_cloudinit.py'
          ,"-f1",pluginpath+"/tools/vmconfig/scvm/hosts","-t1",local_host+$("#div-textarea-cluster-config-confirm-hosts-file-scvm").val() // hosts 파일
    ...
    if(disk_setup_type == "rp"){
      ...
      if(raid_pci_list.length > 0){
        for(var i = 0 ; i < raid_pci_list.length ; i ++ ){
          ...
          el += '<input class="pf-v6-c-check__input" type="checkbox" id="form-checkbox-disk'+i+'" name="form-checkbox-disk" value='+raid_pci_list[i].Slot+' />';
          el += '<label class="pf-v6-c-check__label" style="margin-top:5px" for="form-checkbox-disk'+i+'">'+raid_pci_list[i].Slot+' '+raid_pci_list[i].Vendor+'</label>';
          el += '</div>';
        }
      }else{
        el += '<div class="pf-v6-c-check">';
      ...
      if(lun_pci_list.length > 0){
        for(var i = 0 ; i < lun_pci_list.length ; i ++ ){
          ...
          el += '<div class="pf-v6-c-check">';
          // el += '<input class="pf-v6-c-check__input" type="checkbox" id="form-checkbox-disk'+i+'" name="form-checkbox-disk" value="/dev/'+lun_pci_list[i].name+'" '+check_disable+' />';
          el += '<input class="pf-v6-c-check__input" type="checkbox" id="form-checkbox-disk'+i+'" name="form-checkbox-disk" value="'+lun_pci_list[i].path+'" '+check_disable+' />';
          el += '<label class="pf-v6-c-check__label" style="margin-top:5px" for="form-checkbox-disk'+i+'">/dev/'+lun_pci_list[i].name+' '+lun_pci_list[i].state+' '+lun_pci_list[i].size+' '+lun_pci_list[i].model+' '+partition_text+'</label>';
          el += '</div>';
        }
      }else{
        el += '<div class="pf-v6-c-check">';
...
$('input[name="form-radio-storage-vm-nic-type"]').change(function() {
  ...
  if(storage_traffic_setup_type == "np"){
    ...
    var el1 = '<select class="pf-v6-c-form-control" id="form-select-storage-vm-public-nic2" name="form-select-storage-vm-public-nic2"></select>';
    var el2 = '<select class="pf-v6-c-form-control" id="form-select-storage-vm-cluster-nic2" name="form-select-storage-vm-cluster-nic2"></select>';
...
/**
 * Meathod Name : setScvmOSTypeProcess
 * Date Created : 2024.09.04
 * Writer : 정민철
 * Description : 스토리지센터 가상머신 마법사에서 OS 타입에 대한 문구 및 정보 처리
 * Return : 없음
 * History : 2021.03.30 최초 작성
 */
